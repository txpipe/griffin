...
struct ScriptInfo {
    script_hex: String,
    script_params_cbor: Option<String>,
}
...
struct InputInfo {
    tx_hash: H256,
    index: u32,
    redeemer_cbor: Option<String>, // For script inputs
}
...
pub async fn build_tx(
    ...
    let ordered_inputs_info: Vec<InputInfo> = {
        // Read the inputs info from the JSON file
        let inputs_json: String = std::fs::read_to_string(args.inputs_info)?;
        let mut inputs_info: Vec<InputInfo> = serde_json::from_str(&inputs_json)
            .map_err(|e| anyhow!("Invalid inputs JSON: {}", e))?;
    ...
    // Read the outputs from the JSON file
    let outputs_json: String = std::fs::read_to_string(args.outputs_info)?;
    let transparent_outputs: Vec<TransparentOutput> = serde_json::from_str(&outputs_json)?;
    let outputs: Vec<Output> = transparent_outputs
        .into_iter()
    ...
    let mut mintings_info: Vec<MintingInfo> = Vec::new();
    if args.mintings_info != "" {
        let mintings_info_json: String = std::fs::read_to_string(args.mintings_info)?;
        mintings_info = serde_json::from_str(&mintings_info_json)
            .map_err(|e| anyhow!("Invalid mint info JSON: {}", e))?;
    ...
    let redeemers: Vec<Redeemer> = {
        // Lexicographically order mint info
        let ordered_mintings_info = {
            let mut mintings_info = mintings_info.clone();
            mintings_info.sort_by(|a, b| a.policy.cmp(&b.policy));
            mintings_info
        };

        let minting_redeemers: Vec<Redeemer> = ordered_mintings_info
            .iter()
            .enumerate()
            .map(|(i, mi)| Redeemer {
                tag: RedeemerTag::Mint,
                index: i as u32,
                data: PlutusData(hex::decode(mi.redeemer_cbor.clone()).unwrap()),
            })
            .collect();

        let script_inputs_redeemers: Vec<Redeemer> = ordered_inputs_info
            .iter()
            .enumerate()
            .filter_map(|(i, info)| {
                if let Some(redeemer_cbor) = &info.redeemer_cbor {
                    Some(Redeemer {
                        tag: RedeemerTag::Spend,
                        index: i as u32,
                        data: PlutusData(hex::decode(redeemer_cbor).unwrap()),
                    })
                } else {
                    None
                }
        ...
        script_inputs_redeemers
            .into_iter()
            .chain(minting_redeemers.into_iter())
            .collect()
    };
...
///
/// This command can be replaced with `build_tx` command, but we leave it here
/// since it is simpler for the case of spending wallet inputs.
...
mod tests {
    ...
    fn test_phase2_one_shot_mp() {
        ...
        let resolved_inputs = vec![Output {
            address: Address(
                hex::decode("70".to_owned() + &hex::encode(sender_payment_hash)).unwrap(),
            ),
            value: Value::Coin(10),
            datum_option: None,
        }];
