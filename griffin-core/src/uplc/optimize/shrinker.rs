            ...
            (
                ...
                let (switched, fst_args) = if fst_args.iter_mut().any(|item| item.term == fst.1) {
                    ...
                    fst_args.push(CurriedNode {
                        ...
                        term: core::mem::replace(&mut fst.1, Term::Error.force()),
                    });
...
pub struct Context {
    ...
    pub builtins_map: HashMap<DefaultFunction, ()>,
...
impl Term<Name> {
    ...
    fn replace_identity_usage(&mut self, original: Rc<Name>) {
        match self {
            ...
            Term::Apply { function, argument } => {
                ...
                if *name == original {
                    *self = core::mem::replace(arg, Term::Error.force());
                }
            }
            Term::Force(f) => {
            ...
            Term::Lambda {
                ...
                if let Some(Args::Apply(arg_id, arg_term)) = arg_stack.pop() {
                    ...
                    if replace {
                        ...
                        *self = core::mem::replace(body, Term::Error.force());
                    }
                }
            }
    ...
    fn split_body_lambda(&mut self) {
        ...
        let mut current_term = &mut core::mem::replace(self, Term::Error.force());
        ...
        loop {
            match current_term {
                Term::Apply { function, argument } => {
                    ...
                    arg_stack.push(Args::Apply(0, core::mem::replace(arg, Term::Error.force())));
                }
                Term::Lambda {
                    parameter_name,
                    body,
        ...
        let term_to_build_on = core::mem::replace(current_term, Term::Error.force());
    ...
    fn case_constr_apply_reducer(
        ...
        let mut term = &mut core::mem::replace(self, Term::Error.force());
        ...
        match term {
                ...
                if branches.len() == 1 && matches!(constr.as_ref(), Term::Constr { .. }) =>
            {
                ...
                for arg in arg_vec {
                    fields.push(core::mem::replace(arg, Term::Error.force()));
                }

                *self = core::mem::replace(term, Term::Error.force());
            }
            _ => {
                if arg_vec.len() > 2 {
                    let mut fields = vec![];
                    ...
                    *self = Term::constr(0, fields)
                        .case(vec![core::mem::replace(term, Term::Error.force())]);
                } else {
                    for arg in arg_vec {
                        *term = (core::mem::replace(term, Term::Error.force()))
                            .apply(core::mem::replace(arg, Term::Error.force()));
                    }
    ...
    fn force_delay_reducer(
        ...
        if let Term::Delay(d) = self {
            if let Some(Args::Force(id)) = arg_stack.pop() {
                changed = true;
                context.inlined_apply_ids.push(id);
                *self = core::mem::replace(Rc::make_mut(d), Term::Error.force())
    ...
    fn remove_no_inlines(
        ...
        match self {
            Term::Lambda {
                ...
                *self = core::mem::replace(Rc::make_mut(body), Term::Error.force());
            }
            _ => (),
        }
    }
    ...
    fn inline_constr_ops(
        ...
        if let Term::Apply { function, argument } = self {
            if let Term::Var(name) = function.as_ref() {
                ...
                if name.text == CONSTR_FIELDS_EXPOSER {
                    *self = Term::snd_pair().apply(
                        Term::unconstr_data().apply(core::mem::replace(arg, Term::Error.force())),
                    )
            ...
            } = Rc::make_mut(function)
            {
                ...
                {
                    let body = Rc::make_mut(body);
                    *self = core::mem::replace(body, Term::Error)
                }
            }
        }
    }
        ...
        match self {
            Term::Apply { function, .. } | Term::Force(function) => {
                ...
                if context.inlined_apply_ids.contains(&id) {
                    let func = Rc::make_mut(function);
                    *self = core::mem::replace(func, Term::Error.force());
                }
            }
            _ => (),
        }
    ...
    fn pierce_no_inlines(mut self) -> Self {
        ...
        {
            if parameter_name.as_ref().text == NO_INLINE {
                *term = core::mem::replace(Rc::make_mut(body), Term::Error.force());
            } else {
                break;
            }
        }

        core::mem::replace(term, Term::Error.force())
    }
