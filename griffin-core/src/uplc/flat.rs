    ...
    T: Binder<'b> + Debug,
{
    pub fn from_cbor(bytes: &'b [u8], buffer: &'b mut Vec<u8>) -> Result<Self, de::Error> {
        let mut cbor_decoder = crate::pallas_codec::minicbor::Decoder::new(bytes);
    ...
    ///
    /// ```
    /// use griffin_core::uplc::ast::{Program, Name, Term};
    ///
    ...
    pub fn to_cbor(&self) -> Result<Vec<u8>, en::Error> {
        ...
        let mut cbor_encoder = crate::pallas_codec::minicbor::Encoder::new(&mut bytes);
