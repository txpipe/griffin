...
pub enum Error {
    #[error("{0}")]
    Address(#[from] crate::pallas_addresses::Error),
    #[error("only shelley reward addresses can be a part of withdrawals")]
    BadWithdrawalAddress,
    #[error("{0}")]
    FlatDecode(#[from] crate::pallas_codec::flat::de::Error),
    #[error("{0}")]
    FragmentDecode(#[from] crate::pallas_primitives::Error),
    #[error("{}{}", .0, .2.iter()
        .map(|trace| {
            format!(
                "\n{:>13} {}",
    ...
    WrongEra(#[from] crate::pallas_codec::minicbor::decode::Error),
