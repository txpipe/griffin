...
pub fn validate_conway_tx(
    ...
    let size: u32 = get_conway_tx_size(mtx).ok_or(Conway(UnknownTxSize))?;
...
pub fn check_all_ins_in_utxos(tx_body: &MintedTransactionBody, utxos: &UTxOs) -> ValidationResult {
    for input in tx_body.inputs.iter() {
        if !(utxos.contains_key(&MultiEraInput::from_alonzo_compatible(input))) {
            return Err(Conway(InputNotInUTxO));
        }
    }
        ...
        Some(collaterals) => {
            for collateral in collaterals {
                if !(utxos.contains_key(&MultiEraInput::from_alonzo_compatible(collateral))) {
                    return Err(Conway(CollateralNotInUTxO));
                }
            }
        }
    }
        ...
        Some(reference_inputs) => {
            for reference_input in reference_inputs {
                if !(utxos.contains_key(&MultiEraInput::from_alonzo_compatible(reference_input))) {
                    return Err(Conway(ReferenceInputNotInUTxO));
                }
            }
        }
    }
    ...
    match tx_body.validity_interval_start {
        Some(lower_bound) => {
            if block_slot < lower_bound {
                Err(Conway(BlockPrecedesValInt))
            } else {
                Ok(())
            }
        }
...
fn check_collaterals(
    ...
    let collaterals: &[TransactionInput] = &tx_body
        .collateral
        .clone()
        .ok_or(Conway(CollateralMissing))?;
            ...
            Some(multi_era_output) => {
                if let Some(conway_output) = MultiEraOutput::as_conway(multi_era_output) {
                    ...
                    if let ShelleyPaymentPart::Script(_) =
                        get_payment_part(address).ok_or(Conway(InputDecoding))?
                    {
                        return Err(Conway(CollateralNotVKeyLocked));
                    }
                }
            }
            None => {
        ...
        Some(collaterals) => {
            ...
            let mut coll_input =
                match utxos.get(&MultiEraInput::from_alonzo_compatible(first_collateral)) {
                    ...
                    None => return Err(Conway(CollateralNotInUTxO)),
                };
                ...
                Some(PseudoTransactionOutput::Legacy(output)) => {
                    ...
                    match amount {
                        ...
                        babbage::Value::Multiasset(coin, assets) => {
                            ...
                            for (key, val) in assets.to_vec().into_iter() {
                                let mut conway_value = Vec::new();
                                for (inner_key, inner_val) in val.to_vec().into_iter() {
                                    conway_value.push((
                                        inner_key,
                                        PositiveCoin::try_from(inner_val).unwrap(),
                                    ));
                                }
                                conway_assets.push((key, <_>::try_from(conway_value).unwrap()));
                            }
                            let conway_assets = <_>::try_from(conway_assets).unwrap();
...
pub fn check_preservation_of_value(
    ...
    let output: Value =
        conway_add_values(&produced, &Value::Coin(tx_body.fee), &Conway(NegativeValue))?;
    if let Some(m) = &tx_body.mint {
        input = conway_add_minted_non_zero(&input, m, &Conway(NegativeValue))?;
    }
    if !conway_values_are_equal(&input, &output) {
        return Err(Conway(PreservationOfValue));
    }
    Ok(())
}
...
fn get_consumed(tx_body: &MintedTransactionBody, utxos: &UTxOs) -> Result<Value, ValidationError> {
    ...
    for input in inputs_iter {
        ...
        res = conway_add_values(&res, &val, &Conway(NegativeValue))?;
    }
    Ok(res)
}
    ...
    for output in outputs_iter {
        match output {
            PseudoTransactionOutput::Legacy(output) => {
                ...
                match amount {
                    babbage::Value::Coin(coin) => {
                        res = conway_add_values(&res, &Value::Coin(coin), &Conway(NegativeValue))?;
                    }
                    babbage::Value::Multiasset(coin, assets) => {
                        let mut conway_assets = Vec::new();
                        for (key, val) in assets.to_vec().into_iter() {
            ...
            PseudoTransactionOutput::PostAlonzo(output) => {
                res = conway_add_values(&res, &output.value, &Conway(NegativeValue))?
            }
        }
    }
    Ok(res)
...
fn check_min_lovelace(
    ...
    for output in tx_body.outputs.iter() {
        ...
        if get_lovelace_from_conway_val(val) < compute_min_lovelace(val, prot_pps) {
            return Err(Conway(MinLovelaceUnreached));
        }
    }
    Ok(())
}
    ...
    for output in tx_body.outputs.iter() {
        let val: &Value = match output {
            PseudoTransactionOutput::Legacy(output) => {
                let amount = output.amount.clone();
                match amount {
                    crate::pallas_primitives::babbage::Value::Coin(coin) => &Value::Coin(coin),
                    crate::pallas_primitives::babbage::Value::Multiasset(coin, assets) => {
                        let mut conway_assets = Vec::new();
                        for (key, val) in assets.to_vec().into_iter() {
                            let mut conway_value = Vec::new();
                            for (inner_key, inner_val) in val.to_vec().into_iter() {
        ...
        if conway_get_val_size_in_words(val) > prot_pps.max_value_size as u64 {
            return Err(Conway(MaxValSizeExceeded));
        }
    }
    Ok(())
}
...
fn check_tx_outs_network_id(tx_body: &MintedTransactionBody, network_id: &u8) -> ValidationResult {
    for output in tx_body.outputs.iter() {
        ...
        let addr: ShelleyAddress =
            get_shelley_address(Bytes::deref(addr_bytes)).ok_or(Conway(AddressDecoding))?;
        if addr.network().value() != *network_id {
            return Err(Conway(OutputWrongNetworkID));
        }
    }
    Ok(())
}
...
fn check_tx_network_id(tx_body: &MintedTransactionBody, network_id: &u8) -> ValidationResult {
    if let Some(tx_network_id) = tx_body.network_id {
        if u8::from(tx_network_id) != *network_id {
            return Err(Conway(TxWrongNetworkID));
        }
    }
    Ok(())
}
...
fn check_tx_size(size: &u32, prot_pps: &ConwayProtParams) -> ValidationResult {
    if *size > prot_pps.max_transaction_size {
        return Err(Conway(MaxTxSizeExceeded));
    }
    Ok(())
}
        ...
        match &tx_wits.redeemer {
            Some(redeemers_vec) => {
                ...
                if mem > prot_pps.max_tx_ex_units.mem || steps > prot_pps.max_tx_ex_units.steps {
                    return Err(Conway(TxExUnitsExceeded));
                }
            }
            None => return Err(Conway(RedeemerMissing)),
        }
    }
    Ok(())
}
    ...
    match &tx_body.mint {
        Some(minted_value) => {
            ...
            for (policy, _) in minted_value.iter() {
                ...
                {
                    return Err(Conway(MintingLacksPolicy));
                }
            }
            Ok(())
        }
...
fn check_needed_scripts(
    ...
    for (covered, _) in filtered_native_scripts.iter() {
        if !covered {
            return Err(Conway(UnneededNativeScript));
        }
    }
    for (covered, _) in filtered_plutus_v1_scripts.iter() {
        if !covered {
            return Err(Conway(UnneededPlutusV1Script));
        }
    }
    for (covered, _) in filtered_plutus_v2_scripts.iter() {
        if !covered {
            return Err(Conway(UnneededPlutusV2Script));
        }
    }
...
fn check_input_scripts(
    ...
    for (covered, hash) in needed_input_scripts {
        ...
        {
            return Err(Conway(ScriptWitnessMissing));
        }
    }
    Ok(())
}
    ...
    let mut plutus_data_hashes: Vec<(bool, Hash<32>)> = match option_plutus_data {
        Some(plutus_data) => plutus_data
            ...
            .map(|datum| {
                (
                    false,
                    crate::pallas_crypto::hash::Hasher::<256>::hash(datum.raw_cbor()),
                )
            })
            .collect(),
        None => Vec::new(),
...
fn check_input_datum_hash_in_witness_set(
    ...
    for input in &tx_body.inputs {
            ...
            .and_then(MultiEraOutput::as_conway)
        {
            ...
            None => return Err(Conway(InputNotInUTxO)),
        }
    }
    Ok(())
}
    ...
    for (index, input) in sorted_inputs.iter().enumerate() {
        if get_script_hash_from_input(input, utxos).is_some() {
            res.push(RedeemersKey {
                tag: crate::pallas_primitives::conway::RedeemerTag::Spend,
                index: index as u32,
            })
        }
    }
    if let Some(mint) = &tx_body.mint {
        for (index, policy) in sort_policies(mint).iter().enumerate() {
            if is_phase_2_script(
                ...
                res.push(RedeemersKey {
                    tag: crate::pallas_primitives::conway::RedeemerTag::Mint,
                    index: index as u32,
                })
            }
        }
...
fn check_required_signers(
    ...
    if let Some(req_signers) = &required_signers {
        match &vkey_wits {
            ...
            None => return Err(Conway(ReqSignerMissing)),
        }
    }
    Ok(())
}
    ...
    for vkey_wit in vkey_wits {
        if crate::pallas_crypto::hash::Hasher::<224>::hash(&vkey_wit.vkey.clone()) == *vkey_hash {
            if !verify_signature(vkey_wit, data_to_verify) {
                return Err(Conway(ReqSignerWrongSig));
            } else {
                return Ok(());
            }
        }
    }
    Err(Conway(ReqSignerMissing))
}
...
fn check_vkey_input_wits(
    ...
    let vk_wits: &mut Vec<(bool, VKeyWitness)> =
        &mut mk_alonzo_vk_wits_check_list(vkey_wits, Conway(VKWitnessMissing))?;
    let tx_hash: &Vec<u8> = &Vec::from(mtx.transaction_body.original_hash().as_ref());
        ...
        match utxos.get(&MultiEraInput::from_alonzo_compatible(input)) {
            Some(multi_era_output) => {
                if let Some(babbage_output) = MultiEraOutput::as_conway(multi_era_output) {
                    ...
                    match get_payment_part(address).ok_or(Conway(InputDecoding))? {
...
fn check_vk_wit(
    ...
    for (vkey_wit_covered, vkey_wit) in wits {
        ...
        {
            if !verify_signature(vkey_wit, data_to_verify) {
                return Err(Conway(VKWrongSignature));
            } else {
...
fn check_languages(
    ...
    for tx_lang in tx_languages(mtx, utxos).iter() {
        ...
        {
            return Err(Conway(UnsupportedPlutusLanguage));
        }
    }
    Ok(())
}
    ...
    match (
        ...
        (Some(metadata_hash), Some(metadata)) => {
                ...
                == crate::pallas_crypto::hash::Hasher::<256>::hash(metadata).as_ref()
            {
                ...
                Err(Conway(MetadataHash))
            }
        }
        (None, None) => Ok(()),
        _ => Err(Conway(MetadataHash)),
    }
}
        ...
        Some(script_data_hash) => match (
            ...
            (Some(plutus_data), Some(redeemer)) => {
                ...
                if script_data_hash == indefinite_hash || script_data_hash == definite_hash {
                    ...
                    Err(Conway(ScriptIntegrityHash))
                }
            }
            (None, Some(redeemer)) => {
            ...
            (None, None) => Err(Conway(ScriptIntegrityHash)),
            (Some(_), None) => Err(Conway(ScriptIntegrityHash)),
        },
        None => {
            let plutus_data = mtx
                        ...
                        .unwrap()
                    {
                        Redeemers::List(redeemers) => {
                            if !option_vec_is_empty(&Some(redeemers.clone().to_vec())) {
                                return Err(Conway(ScriptIntegrityHash));
                            }
                        }
...
fn compute_script_integrity_hash(
    ...
    (
        crate::pallas_crypto::hash::Hasher::<256>::hash(&value_to_hash_with_indef),
        crate::pallas_crypto::hash::Hasher::<256>::hash(&value_to_hash_with_def),
    )
}
