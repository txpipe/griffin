...
fn coerce_to_u64(value: &ConwayMultiasset<PositiveCoin>) -> ConwayMultiasset<u64> {
    ...
    for (policy, assets) in value.iter() {
        ...
        res.push((
            *policy,
            NonEmptyKeyValuePairs::<AssetName, u64>::try_from(aa)
                .expect("Failed to convert to NonEmptyKeyValuePairs"),
        ));
    }
    NonEmptyKeyValuePairs::<PolicyId, NonEmptyKeyValuePairs<AssetName, u64>>::try_from(res)
        .expect("Failed to convert to NonEmptyKeyValuePairs.")
}
...
fn conway_coerce_to_coin(
    ...
    Ok(NonEmptyKeyValuePairs::<PolicyId, NonEmptyKeyValuePairs<AssetName, PositiveCoin>>::try_from(res).unwrap())
}
